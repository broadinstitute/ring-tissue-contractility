...
def motion_analysis(file_path, output_folder, pixel_size_um, diameter_um, E, frame_rate = 100):
    """
    ...
    Also calculates normalized ring intensity fluctuation over time and plots it
        - get the area of just the ring (use the coverage metric code), and erode away a little
        - get the average intensity of the ring area for that frame
        - get the average intensity of the entire movie
        - for each frame calculate the (intensity - mean intensity) / mean intensity
    ...
    n = nd2_file.shape[0]  # Number of frames
    h = nd2_file.shape[1]  # Height
    w = nd2_file.shape[2]  # Width
    ...
    gaussian_kernel_size = 7     # Large kernel for aggressive erosion (ring-only mask subtraction)
    ring_inner_kernel = np.ones((15, 15), np.uint8)
    ...
    for i in range(video_frames.shape[2]):
        ...
        # Calculate Otsu's threshold for the enhanced frame
        current_bw_threshold = threshold_otsu(processed_frame_8bit)
        
        _, mask_full = cv2.threshold(processed_frame_8bit, current_bw_threshold, 255, cv2.THRESH_BINARY)
        mask_full = cv2.morphologyEx(mask_full, cv2.MORPH_CLOSE, morph_kernel, iterations=4)

        # Inner pillar mask- Aggressive erosion
        mask_inner = cv2.erode(mask_full, ring_inner_kernel, iterations=1) 
        
        # Subtract pillar from entire mask
        mask_ring_255 = cv2.subtract(mask_full, mask_inner)
        mask_ring_bool = mask_ring_255.astype(bool)

        # Calculate ring intensity
        frame_intensity_data = video_frames[:, :, i] 
        ring_pixels = frame_intensity_data[mask_ring_bool]
        
        if ring_pixels.size > 0:
            ring_intensities[i] = np.mean(ring_pixels)
        else:
            logging.warning(f"Ring mask is empty for frame {i}. Setting ring intensity to NaN.")
            ring_intensities[i] = np.nan
        
        # Save mask
        if i == 0:
            fig, ax = plt.subplots(1, 2, figsize=(8, 8))
            ax[0].imshow(enhanced_frame, cmap='gray')
            ax[0].set_title("Processed image")
            ax[0].axis('off')
            ax[1].imshow(mask_full, cmap='gray')
            ax[1].set_title(f"Mask: {name}")
            ax[1].axis('off')
        ...
        # Convert the boolean array (True/False) to an 8-bit integer array (255/0)
        mask_full = (mask_full * 255).astype(np.uint8)
        
        # Find all contours and their hierarchy
        contours, _ = cv2.findContours(mask_full, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        ...
        if len(contours) >= 2:
            ...
            # Reapply thresholding to the processed frame for coverage metric calculation
            # Adding more preprocessing steps to avoid background noise interfering with contour detection
            _, bw = cv2.threshold(processed_frame_8bit, current_bw_threshold, 255, cv2.THRESH_BINARY)
            
            # Morphological Opening to remove small noise artifacts/dots
            bw = cv2.morphologyEx(bw, cv2.MORPH_OPEN, morph_kernel, iterations=1)
            
            #  Morphological Closing to close small gaps in the main tissue
            bw = cv2.morphologyEx(bw, cv2.MORPH_CLOSE, morph_kernel, iterations=1)
            ...
            bounding_circle_mask = np.zeros_like(mask_full)
            ...
            tissue_mask = np.zeros_like(mask_full)
    ...
    # Calculate overall movie mean intensity (using only valid ring intensity frames)
    valid_indices = ~np.isnan(ring_intensities)
    valid_frames_data = video_frames[:, :, valid_indices]

    if valid_frames_data.size == 0:
        logging.warrning("\nAnalysis failed: No valid ring intensity data found")
        mean_movie_intensity = np.nan
        normalized_intensity = np.full_like(ring_intensities, np.nan)
    else:
        mean_movie_intensity = np.mean(valid_frames_data)
        logging.info(f"Overall Movie Mean Intensity (I): {mean_movie_intensity:.4f}")
        
        # Calculate the normalized intensity (Delta I / I)
        normalized_intensity = np.full_like(ring_intensities, np.nan)
        normalized_intensity[valid_indices] = (ring_intensities[valid_indices] - mean_movie_intensity) / mean_movie_intensity
    ...
    raw_data = pd.DataFrame({
        ...
        'Ring_Intensity_Normalized': normalized_intensity
    })
    ...
    b, a = butter(2, fc /fs, 'low', analog=False, fs=fs)
