...
def calculate_rise_fall_times(signal_series, time_series):
    ...
    if np.mean(np.diff(valid_data['time'])) > 0:
        min_dist_samples = int(1 / 0.1 / np.mean(np.diff(valid_data['time'])))   #assuming a min frequency of 0.1 Hz (should be fine since these typically get paced at 1 Hz)
    else:
        min_dist_samples = 1
...
def motion_analysis(input_folder, output_folder, pixel_size_um, diameter_um, E, frame_rate = 100):
    """
    Takes in a folder of .nd2 movies, processes them, 
    and calculates various metrics (force, stress, strain, contraction and relaxation speed, etc.)
    For each movie: saves 3 CVS files, a plot of the mask, and a runtime log in the output folder
    
    Args:
        input_folder (str): The path to the folder containing .nd2 movie files
    ...
    logging.basicConfig(
        ...
        filename=os.path.join(output_folder,'runtime_log.txt'), 
        filemode='w'         
    )
    ...
    # Loop through the folder and process the .nd2 files
    for video in os.listdir(input_folder):
        if video.endswith(".nd2"):
            ...
            for i in range(video_frames.shape[2]):
                ...
                processed_frame_8bit = cv2.GaussianBlur(frame_8bit, (gaussian_kernel_size, gaussian_kernel_size), 0)

                # Threshold the image using the threshold determined above
                _, bw = cv2.threshold(np.uint8(frame * 255), bw_threshold, 255, cv2.THRESH_BINARY_INV)
                
                # Save a plot of the first mask for reference
                if i == 0:   
                    fig, ax = plt.subplots(figsize=(8, 8))
                    ax.imshow(bw, cmap='gray')
                    ax.set_title(f"Mask of: {name}")
                    ax.axis('off')
